...
def _find_identity_dir(serve_root: Path, name: str) -> Path:
    for base in (".identities", "identities"):
        ...
        if (candidate / "info.txt").is_file():
            return candidate
    raise SystemExit(
        f"shelly-telegram-bridge: identity '{name}' not found under {serve_root} "
...
def load(serve_root: Path) -> Config:
    ...
    if not bot_token:
        raise SystemExit("shelly-telegram-bridge: TELEGRAM_BOT_TOKEN is required")
    admin_raw = os.environ.get("TELEGRAM_ADMIN_ID", "")
    if not admin_raw.isdigit():
        raise SystemExit(
            "shelly-telegram-bridge: TELEGRAM_ADMIN_ID is required (your numeric "
            "Telegram user id — message the bot once and check the bridge log)"
        )
    # SHELLY_* is canonical; SHELLM_* accepted as a legacy fallback until the
    # deployed boxes migrate their env files.
    identity = (
        os.environ.get("SHELLY_TELEGRAM_IDENTITY")
        or os.environ.get("SHELLM_TELEGRAM_IDENTITY")
        or "audel"
    )
    identity_dir = _find_identity_dir(serve_root, identity)
    state_dir = Path(
        os.environ.get("SHELLY_TELEGRAM_STATE_DIR")
