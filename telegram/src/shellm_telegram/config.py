...
def _find_identity_dir(serve_root: Path, name: str) -> Path:
    for base in (".identities", "identities"):
        ...
        if (candidate / "info.txt").is_file():
            return candidate
    raise SystemExit(
        f"shellm-telegram-bridge: identity '{name}' not found under {serve_root} "
...
def load(serve_root: Path) -> Config:
    ...
    if not bot_token:
        raise SystemExit("shellm-telegram-bridge: TELEGRAM_BOT_TOKEN is required")
    admin_raw = os.environ.get("TELEGRAM_ADMIN_ID", "")
    if not admin_raw.isdigit():
        raise SystemExit(
            "shellm-telegram-bridge: TELEGRAM_ADMIN_ID is required (your numeric "
            "Telegram user id — message the bot once and check the bridge log)"
        )
    identity = os.environ.get("SHELLM_TELEGRAM_IDENTITY", "audel")
    identity_dir = _find_identity_dir(serve_root, identity)
    state_dir = Path(
        os.environ.get(
            "SHELLM_TELEGRAM_STATE_DIR", identity_dir / "run" / "telegram-bridge"
        )
    )
    ...
    return Config(
        ...
        web_url=os.environ.get("SHELLM_WEB_URL", "http://127.0.0.1:8080").rstrip("/"),
        state_dir=state_dir,
    )
