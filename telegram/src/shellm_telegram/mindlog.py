...
Copied from shellm_slack.mindlog (the bridges are independent uv
...
def find_trajectory(identity_dir: Path) -> Path:
    """Locate the mind log, mirroring shellm_web.discovery.find_root_traj_dir."""
    info = {}
    ...
    raise SystemExit(f"shellm-telegram-bridge: no trajectory.jsonl under {traj_root}")
