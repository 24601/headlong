...
- ~~No guard yet against two concurrent sessions in the same generation colliding on run numbering~~ **FIXED (2026-08-08):** `session.sh` now wraps run_num selection + `identity new` in a `flock`-guarded block on `$GEN_DIR/.session.lock` (commit `7f345be`). Lock held only for slot assignment; sessions still concurrent afterward. **TESTED (2026-08-08):** `improve/test/concurrent-session-flock.sh` runs 8 and 20 parallel workers against the real flock block — all get distinct contiguous run_nums (1..N), no collisions. Flock fix verified correct.
