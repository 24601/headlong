...
- 2026-08-08 gen-003 ACCEPTED fix-memory-index-rebuild → bin/mem + thinkers/retrieval/build_index.sh — MOOT: the keyword-index retrieval approach (build_index.sh + retrieval thinker) was removed from production entirely. bin/mem now uses LLM-based semantic search (action_search concatenates all memories, LLM ranks them). bin/mem-search-semantic was also removed. No index rebuild needed — retrieval is index-free. Verified: bin/mem has zero index/rebuild/build_index references (2026-08-17).
