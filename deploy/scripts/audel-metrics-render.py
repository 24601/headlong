...
The page shows the reply-latency baseline: a KPI row, then two ECDF panels
("share of replies delivered within X") on a log time axis — split by reply
path (fast vs in-run) and by arrival condition (idle vs during a run).
Stamped reply pairs only, same definition as the text report.
"""
import json
import math
import sys
...
def main():
    ...
    html = HTML_TEMPLATE
    for token, value in (
            ("@@TITLE@@", "Audel reply latency baseline"),
...
<h1>@@TITLE@@</h1>
<p class="sub">Share of replies delivered within a given time. Stamped reply pairs from the full
mind log, pulled @@GENERATED@@. Log time axis.</p>
