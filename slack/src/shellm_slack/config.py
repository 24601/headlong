...
def _find_identity_dir(serve_root: Path, name: str) -> Path:
    for base in (".identities", "identities"):
        ...
        if (candidate / "info.txt").is_file():
            return candidate
    raise SystemExit(
        f"shellm-slack-bridge: identity '{name}' not found under {serve_root} "
...
def load(serve_root: Path) -> Config:
    ...
    identity = os.environ.get("SHELLM_SLACK_IDENTITY", "audel")
    identity_dir = _find_identity_dir(serve_root, identity)
    state_dir = Path(
        os.environ.get("SHELLM_SLACK_STATE_DIR", identity_dir / "run" / "slack-bridge")
    )
