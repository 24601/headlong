"""Launch the Shelly dash (web viewer)

Prod:  shelly-web [ROOT]         # build frontend if missing, serve on one port
Dev:   shelly-web --dev [ROOT]   # vite dev server + uvicorn --reload
"""
    ...
    for port in ports:
        if _port_free(host, port):
            if port != ports.start:
                ...
                print(
                    f"shelly-web: WARNING: port {ports.start} is taken"
...
def _js_runtime() -> list[str]:
    ...
    Order: $SHELLY_WEB_JS override (legacy $SHELLM_WEB_JS), then
    ...
    override = getenv("SHELLY_WEB_JS")
    if override:
        runtime = resolve(override)
        if runtime is None:
            raise SystemExit(f"SHELLY_WEB_JS={override} but it isn't on PATH")
        return runtime
...
def _run_production(root: Path, host: str, port: int, rebuild: bool, read_only: bool) -> None:
    ...
    # NOTE: tools/shelly-init greps the log for "shell(m|y)-web serving"
...
def _run_dev(root: Path, host: str, port: int, read_only: bool) -> None:
    ...
    print(
        f"shelly-web dev: backend http://{host}:{port}, frontend http://localhost:5173",
        file=sys.stderr,
    )
    try:
        os.environ["SHELLY_WEB_ROOT"] = str(root)
        if read_only:
            os.environ["SHELLY_WEB_READONLY"] = "1"
        uvicorn.run(
            "shelly_web:create_app_from_env",
            host=host,
            port=port,
            factory=True,
            reload=True,
...
def main() -> None:
    parser = argparse.ArgumentParser(prog="shelly-web", description=__doc__)
    ...
    if args.port is not None:
        if not _port_free(args.host, args.port):
            ...
            raise SystemExit(
                f"shelly-web: port {args.port} is already in use"
                + (f" by {owner}" if owner else "")
                + " — is another shelly-web still running?"
            )
        port = args.port
    else:
        port = _find_available_port(args.host, DEFAULT_PORT_RANGE)
    read_only = args.read_only or getenv("SHELLY_WEB_READONLY", "") not in ("", "0")
