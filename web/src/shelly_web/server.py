"""FastAPI app factory for the Shelly dash (web viewer)."""
...
from shelly_web.health import response_stats

logger = logging.getLogger(__name__)

VERSION = "0.1.0"

# The repo the running server code lives in (…/web/src/shelly_web/server.py)
...
def create_app(
    ...
    root = root.resolve()
    app = FastAPI(title="Shelly dash", version=VERSION)
    # Default "*" suits local use; deployments should pin this to their
    # public origin(s) via SHELLY_WEB_ALLOWED_ORIGINS (comma-separated).
    allowed_origins = [
        origin.strip()
        for origin in env.getenv("SHELLY_WEB_ALLOWED_ORIGINS", "*").split(",")
        if origin.strip()
    ]
    app.add_middleware(
        CORSMiddleware,
    ...
    self_update_enabled = (
        env.getenv("SHELLY_WEB_SELF_UPDATE") == "1" and not read_only
    )
    ...
    def self_update() -> dict:
        ...
        if not self_update_enabled:
            raise HTTPException(
                status_code=403,
                detail="Self-update is disabled (set SHELLY_WEB_SELF_UPDATE=1)",
            )
    ...
    async def import_identities(
        ...
        max_bytes = int(env.getenv("SHELLY_WEB_MAX_IMPORT_MB", "512")) * 1024 * 1024
            ...
            with os.fdopen(fd, "wb") as out:
                async for chunk in request.stream():
                    ...
                    if total > max_bytes:
                        raise HTTPException(
                            status_code=413,
                            detail=f"Archive exceeds SHELLY_WEB_MAX_IMPORT_MB ({max_bytes // (1024 * 1024)} MB)",
                        )
                    out.write(chunk)
