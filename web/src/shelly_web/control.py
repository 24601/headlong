...
from shelly_web import envfile
from shelly_web.discovery import IdentityInfo, _parse_info_txt
from shelly_web.env import getenv

# Repo layout: <repo>/web/src/shelly_web/control.py -> <repo>/bin
BIN_DIR = Path(
...
def _thinkersctl_path() -> str:
    return getenv("SHELLY_THINKERSCTL", "")


def _thinkersctl(
    action: str, identity_name: str, timeout: int = DEFAULT_TIMEOUT
) -> subprocess.CompletedProcess:
    # SHELLY_THINKERSCTL_SUDO (legacy SHELLM_THINKERSCTL_SUDO) exists for
    # tests, which can't sudo; empty string means "run the wrapper directly".
    sudo = getenv("SHELLY_THINKERSCTL_SUDO", "sudo -n").split()
